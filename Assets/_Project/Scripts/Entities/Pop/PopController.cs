...
using Lineage.Debug; // For runtime Log system
...
namespace Lineage.Entities
{
    ...
    public class PopController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // TODO: EntityData access disabled - namespace conflict between Lineage.Entities.Stat and Lineage.Database.Stat
        ...
        {
            if (isSelected == selected) return; // No change needed, skip redundant updates

            isSelected = selected;

            // Update visual feedback - color tint and sorting order
            if (_popSpriteRenderer != null)
            {
                if (selected)
                {
                    // Store original color on first selection
                    ...
                    // Apply selection highlight (30% brighter)
                    ...
                    // Bring selected pop forward in sorting order for visual prominence
                ...
                {
                    // Deselect: restore original color and sorting order
        ...
        public void StopMovement()
        {
            ...
            {
                // Only set animator bool if parameter exists to avoid warnings
                if (HasAnimatorParameter(pop.Animator, "IsMoving"))
                {
        ...
        private void Update()
        {
            ...
            {
                // Not fully initialized or agent disabled
                return;
            }
            ...
            if (popAnimator == null)
            {
                return; // Missing animator, can't update animation state
            }
        ...
        {
            // Get SelectionManager singleton and add this pop to selection
            ...
            {
                return; // Already selected or list is invalid
            }
        ...
        /// <summary>
        /// Helper method to check if an animator has a specific parameter
        /// Prevents warnings when trying to set parameters that don't exist
        /// </summary>
